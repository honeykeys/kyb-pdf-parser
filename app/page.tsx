...
const styles: { [key: string]: React.CSSProperties } = {
  mainContainer: {
    ...
    minHeight: '100vh',
    padding: '20px',
    backgroundColor: 'var(--pooh-yellow)',
    color: 'var(--pooh-text-dark)',  
    fontFamily: 'var(--font-inter, Arial, sans-serif)', 
  },
  card: {
    width: '100%',
    maxWidth: '800px',
    backgroundColor: 'var(--pooh-input-bg)', 
    boxShadow: '0 4px 12px rgba(0,0,0,0.1)',
    borderRadius: '12px',
    padding: '30px',
    border: '2px solid var(--pooh-input-border)',
  },
  header: {
    textAlign: 'center',
    marginBottom: '30px',
  ...
  fileInputButton: {
    ...
    alignItems: 'center',
    padding: '10px 18px',
    backgroundColor: 'var(--pooh-red)',
    color: 'var(--pooh-text-on-dark)',
    borderRadius: '8px',
    cursor: 'pointer',
    border: 'none',
    fontSize: '1rem',
  ...
  fileInputButtonHover: {
    backgroundColor: 'var(--pooh-button-hover)',
  },
  fileName: {
    marginLeft: '12px',
    color: 'var(--pooh-text-dark)',
  ...
  buttonDisabled: {
    backgroundColor: 'var(--pooh-disabled-bg)',
    color: 'var(--pooh-disabled-text)',
    cursor: 'not-allowed',
  },
  loadingSpinner: {
    width: '20px',
    ...
    borderTopColor: 'var(--pooh-text-on-dark)',
    borderRadius: '50%',
  ...
  errorBox: {
    marginTop: '20px',
    padding: '15px',
    backgroundColor: 'var(--pooh-error-bg)',
    color: 'var(--pooh-error-text)',     
    borderRadius: '8px',
    border: '1px solid var(--pooh-red)',
  },
  resultsContainer: {
    marginTop: '30px',
    padding: '25px',
    ...
    borderRadius: '8px',
    border: '1px solid var(--pooh-input-border)',
  },
  resultsTitle: {
    fontSize: '1.75rem',
    fontWeight: 'bold',
    color: 'var(--pooh-text-header)',
    borderBottom: '2px solid var(--pooh-input-border)',
    paddingBottom: '10px',
    marginBottom: '20px',
  },
  dataItem: {
  ...
  preformattedText: {
    backgroundColor: 'var(--pooh-table-row-alt)',
    padding: '12px',
    borderRadius: '6px',
    overflowX: 'auto',
    maxHeight: '600px',
  ...
  th: {
    borderBottom: '2px solid var(--pooh-text-header)',
    padding: '12px 10px',
    textAlign: 'left',
    backgroundColor: 'var(--pooh-table-header-bg)',
    color: 'var(--pooh-text-dark)',
    fontSize: '0.9rem',
    fontWeight: 'bold',
    textTransform: 'uppercase',
  ...
  td: {
    borderBottom: '1px solid var(--pooh-input-border)', 
    padding: '10px 10px',
    fontSize: '0.95rem',
    color: 'var(--pooh-text-dark)',
  },
  transactionAmountNegative: {
    color: 'var(--pooh-error-bg)',
    fontWeight: 'bold',
  },
  transactionAmountPositive: {
    color: 'var(--pooh-success-bg)', 
    fontWeight: 'bold',
  },
  reconciliationBox: {
    marginTop: '25px',
    ...
    borderRadius: '8px',
    color: 'var(--pooh-text-on-dark)',
    borderTop: '1px solid var(--pooh-input-border)',
    paddingTop: '20px',
  },
  reconciliationMatch: {
    backgroundColor: 'var(--pooh-success-bg)',
  },
...
export default function HomePage() {
  ...
  const fileInputButtonStyle = {
    ...
    ...(isButtonHovered ? styles.fileInputButtonHover : {})
  };

  const submitButtonStyle = {
    ...styles.submitButton,
    ...((isLoading || !file) ? styles.buttonDisabled : (isSubmitHovered ? styles.fileInputButtonHover : {}))
  };
            ...
            {apiResponse.extractedData && (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '20px' }}>
                  ...
                  <div style={styles.dataItem}><span style={styles.dataLabel}>Starting Balance:</span> <span style={styles.dataValue}>{formatCurrency(apiResponse.extractedData.startingBalance)}</span></div>
                  <div style={styles.dataItem}><span style={styles.dataLabel}>Ending Balance (Stated):</span> <span style={styles.dataValue}>{formatCurrency(apiResponse.extractedData.endingBalance)}</span></div>
                </div>
                  ...
                  {apiResponse.extractedData.transactions && apiResponse.extractedData.transactions.length > 0 ? (
                    <div style={{ maxHeight: '400px', overflowY: 'auto', border: '1px solid var(--pooh-input-border)', borderRadius: '6px' }}>
                      <table style={styles.table}>
                        <thead style={{position: 'sticky', top: 0, zIndex: 1, backgroundColor: 'var(--pooh-table-header-bg)' /* Ensure header bg is part of sticky element */}}>
                          <tr>
                    ...
                    <p style={{ fontStyle: 'italic', color: 'var(--pooh-text-dark)', opacity: 0.8 }}>
            ...
            {apiResponse.reconciliation && apiResponse.extractedData && (
              <div style={{ ...styles.reconciliationBox, ...(apiResponse.reconciliation.matches ? styles.reconciliationMatch : styles.reconciliationMismatch) }}>
                ...
                <div style={{ fontSize: '0.95rem', color: 'var(--pooh-text-on-dark)' }}>
                  <p>Stated Ending Balance: <strong>{formatCurrency(apiResponse.extractedData.endingBalance)}</strong></p>
                  <p>Calculated Ending Balance (Start + Transactions): <strong>{formatCurrency(apiResponse.reconciliation.calculatedEndingBalance)}</strong></p>
                  <p>Difference: <strong style={apiResponse.reconciliation.difference !== 0 && !apiResponse.reconciliation.matches ? { color: 'var(--pooh-yellow)'} : {}}>{formatCurrency(apiResponse.reconciliation.difference)}</strong></p>
                </div>
              </div>
            )}
          </div>
