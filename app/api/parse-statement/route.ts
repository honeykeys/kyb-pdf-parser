...
async function parsePdfToString(fileBuffer: ArrayBuffer): Promise<string> {
  try {
    ...
    if (!data.text || data.text.trim() === "") {
      console.warn("API Warning: PDF parsed, but no text content was extracted.");
    }
    return data.text;
  } catch (error) {
    console.error("API Error: pdf-parse encountered an error.", error);
...
async function getDetailsFromLlm(text: string): Promise<Omit<ExtractedData, 'rawPdfText'>> {
  ...
  if (!GOOGLE_API_KEY || GOOGLE_API_KEY === "YOUR_API_KEY_NOT_SET") {
    console.error("API Error: GOOGLE_API_KEY is not configured.");
    return {
      accountHolderName: "Error: Gemini API Key Missing",
      accountHolderAddress: null, statementDate: null, currency: null,
      transactions: [], startingBalance: null, endingBalance: null,
    };
  }
  if (!text || text.trim() === "") {
    console.warn("API Warning: Gemini LLM called with empty text.");
    return {
      accountHolderName: null, accountHolderAddress: null, statementDate: null, currency: null,
      transactions: [], startingBalance: null, endingBalance: null,
    };
  }
...
The JSON object must strictly follow this schema:
{
  ...
  "currency": "string (e.g., GBP, USD, EUR) | null (The main currency of the statement, use ISO 4217 codes)",
...
2.  Currency Identification: Identify the primary currency used in the statement (e.g., GBP, USD, EUR). Return this as a three-letter ISO 4217 currency code in the 'currency' field. If no currency is explicitly found, attempt to infer it from common symbols (£ for GBP, $ for USD, € for EUR) or return null if ambiguous.
3.  Amounts: Ensure 'amount' for transactions, 'startingBalance', and 'endingBalance' are numbers. These monetary values MUST be returned as numbers only, without any currency symbols (£, $, €) or commas as thousands separators. Debits, withdrawals, payments, or charges should be negative. Deposits, credits, or payments received should be positive.
4.  Interpreting Debits/Overdrafts: If an amount is followed by "OD", "DR", or is in a dedicated debit column, it represents a negative value. Convert these to a negative number for the 'amount' field (e.g., "50.00 OD" becomes -50.00). If an amount is explicitly shown with a minus sign (e.g., "-50.00" or "(50.00)"), treat it as negative.
5.  Balances: 'startingBalance' and 'endingBalance' should be numbers, without currency symbols. If not found, return null.
6.  Handling Balance Forward: Lines like 'BALANCE BROUGHT FORWARD', 'B/FWD', 'OPENING BALANCE', or similar descriptions that appear in the transaction list area should NOT be treated as new transactions if their specific debit/credit amount columns are empty, zero, or not present. The balance figure on such a line often represents the 'startingBalance' for the period. If you use such a line to determine the 'startingBalance', do not also list it as a transaction.
7.  Transactions: Extract all itemized transactions. If a transaction line has separate debit and credit columns, combine them into a single 'amount' field with the correct sign. Be careful to distinguish between actual transactions and summary lines or running balances within the transaction list.
8.  Address: Concatenate multi-line addresses into a single string, preferably with newline characters preserved (e.g., "123 Main St\\nAnytown, ST 12345"). If preserving newlines as \`\\n\` in the JSON string is difficult, using spaces is acceptable.
9.  Missing Information: If a specific piece of information (e.g., startingBalance, currency) is not found or cannot be reliably inferred, return \`null\` for that field. Do not make up information.
10. Accuracy: Be as accurate as possible. Double-check numbers, dates, and currency codes.
11. JSON Only: Your entire response must be ONLY the JSON object. No introductory phrases, no concluding remarks, just the JSON. Do not wrap the JSON in markdown backticks.
12. Empty Arrays: If no transactions are found, return an empty array for the "transactions" field: \`"transactions": []\`.
13. Null for Top-Level Fields: If a top-level field like \`accountHolderName\` is not found, it should be \`null\`.
...
`;
  ...
  try {
    ...
    const requiredFields = ["accountHolderName", "accountHolderAddress", "statementDate", "currency", "startingBalance", "endingBalance", "transactions"];
    for (const field of requiredFields) {
        if (!(field in parsedJson)) {
            console.warn(`API Warning: Gemini JSON response missing expected field: ${field}. Defaulting.`);
            if (field === "transactions") parsedJson.transactions = [];
            else parsedJson[field] = null;
        }
    }
    if (parsedJson.transactions && !Array.isArray(parsedJson.transactions)) {
        console.warn("API Warning: Gemini 'transactions' field is not an array. Defaulting.");
    ...
    return {
      accountHolderName: `Error: Gemini LLM Failed (Details: ${error.message || 'Unknown'})`,
      accountHolderAddress: null, statementDate: null, currency: null,
      transactions: [], startingBalance: null, endingBalance: null,
    };
  }
...
function performReconciliation(data: Omit<ExtractedData, 'rawPdfText'>): ReconciliationResult {
  ...
  if (data.startingBalance === null || data.endingBalance === null || !data.transactions) {
    ...
    return { calculatedEndingBalance: null, difference: null, matches: false };
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!extractedText || extractedText.trim().length < 50) {
        console.warn("API Warning: Extracted text is very short or empty.");
        return NextResponse.json({
            extractedData: {
                ...
                statementDate: "N/A", currency: null, transactions: [], startingBalance: null, endingBalance: null,
